...
namespace Microsoft.MixedReality.WebRTC.Unity
{
    ...
    {
        private byte[] pointerArray = new byte[1 + 2 * 4];
        public static byte[] incomingGesture;
        public static byte[] incomingDepthPacket;
        public static byte[] incomingAnnotationPacket;
        public static byte[] incomingPointerPacket;
        public Vector3 trackerPosition;
        public Quaternion trackerRotation;
        public static Queue<byte[]> AnnotationTasks = new Queue<byte[]>();
        public Queue<byte[]> selectCameraPacket = new Queue<byte[]>();
        public DepthData DepthData = new DepthData();
        public byte selectCamera = 0;
        public WebRTCManager WebRTCManager;
        public AnnotationData AnnotationData = new AnnotationData();
        public byte alivePacketCount;
        public bool sendMessages = true;
        public DataChannel myDataChannelOut;
        public DataChannel myDataChannelIn;
        /// <summary>
        ...
        public string DataChannelName = "null";
        ...
        public List<ConfigurableIceServer> IceServers = new List<ConfigurableIceServer>()
        {
           ...
           /*
            ...
            {
                Type = IceType.Turn,
                Uri = "turn.because-why-not.com:443"
            } */
        };
        ...
        {
            Debug.Log( "WebRTC plugin initialized successfully: " + WebRTCManager.signaler.LocalPeerId);
        ...
        public void AddDataChannels()
        {
            ushort id;                                                          
            string label;
            if (!CommonParameters.useMultipleDataChannels)
            {
                //id = CommonParameters.connections[DataChannelName].id;
                //label = CommonParameters.connections[DataChannelName].label;
                //AddDataChannel(myDataChannelOut, id, label, "");
                id = CommonParameters.connections[DataChannelName].id;
                label = CommonParameters.connections[DataChannelName].label;
                AddDataChannelOut(id, label);
                
                //myDataChannelOut = myDataChannelIn;
            }
            else
            {
                id = CommonParameters.connections[PeerName + "In"].id;
                label = CommonParameters.connections[PeerName + "In"].label;
                AddDataChannelIn(id, label);

                id = CommonParameters.connections[PeerName + "Out"].id;
                label = CommonParameters.connections[PeerName + "Out"].label;
                AddDataChannelOut(id, label);
            }
        }

        //public void AddDataChannel(DataChannel dataChannel, ushort id, string label, string type)
        public void AddDataChannelOut(ushort id, string label)
        {
            if (myDataChannelOut != null)
            {
                try
                {
                    print("AddDataChannel(): Data Channel already exits!");
                    return;
                    _nativePeer.RemoveDataChannel(myDataChannelOut);  
                }
                catch (Exception e)
                {
                    //print("MR: RemoveDataChannel ERROR");
                    Console.WriteLine(e);
                    //throw;
                }
                myDataChannelOut = null;
            }
            ...
            //_nativePeer.AddDataChannelAsync(13, "chat", true, true).ContinueWith((prevTask) => // ID..new unique id
            _nativePeer.AddDataChannelAsync(id, label, false, CommonParameters.dataChannelIsReliable).ContinueWith((prevTask) => // ID..new unique id
            {
                if (prevTask.Exception != null)
                {
                ...
                //RunOnMainThread(() =>
                {
                    myDataChannelOut = newDataChannel;
                     if(!CommonParameters.useMultipleDataChannels)
                    {
                        myDataChannelOut.MessageReceived += ChatMessageReceived;
                    }

                    myDataChannelOut.BufferingChanged += monitorBuffer;
                    //chatInputBox.IsEnabled = true;
                    //chatSendButton.IsEnabled = true;
                }
                //);
            });
            print("Data Channel Out added!");
        }
        
        public void AddDataChannelIn(ushort id, string label)
        {
            if (myDataChannelIn != null)
            {
                try
                {
                    print("AddDataChannel(): Data Channel already exits!");
                    return;
                    _nativePeer.RemoveDataChannel(myDataChannelIn);  
                }
                catch (Exception e)
                {
                    //print("MR: RemoveDataChannel ERROR");
            ...
            _nativePeer.AddDataChannelAsync(id, label, false, CommonParameters.dataChannelIsReliable).ContinueWith((prevTask) => // ID..new unique id
            {
                ...
                //RunOnMainThread(() =>
                {
                    myDataChannelIn = newDataChannel;
                    // (type == "in")
                    {
                        myDataChannelIn.MessageReceived += ChatMessageReceived;
                    }

                    myDataChannelIn.BufferingChanged += monitorBuffer;
                    //chatInputBox.IsEnabled = true;
                    //chatSendButton.IsEnabled = true;
                }
                //);
            });
            print("Data Channel In added!");
        }

        private void monitorBuffer(ulong previousBufferSizeBytes, ulong currentBufferSizeBytes, ulong maxBufferSizeBytes)
        {
            print( PeerName+ ": Prev: " + previousBufferSizeBytes + " Curr: " + currentBufferSizeBytes + " Max: " + maxBufferSizeBytes);

            //sendMessages = false;
            //print("Don't send messages anymore!!!");
        }
        public void SendData(byte[] data)
        {
            myDataChannelOut.SendMessage(data);
        }
        ...
        public void SendMessage(string message)
        {
            ...
            if(message == "shutdown")
            {
                ...
                //myDataChannel.SendMessage(new byte[] {0x00});
            } else if (message == "depth")
            {
                if (PeerName == "LocalComputer2Remote")
                {
                    //print("skip send message");
                    //return;
                }
                ...
                if (DepthData.DepthPackets.Count > 0)
                {
                    ...
                    //print("send size: " + sendBytes.Length);
                }
                else
                {
                    return;
            ...
            var compressed = Compress(sendBytes);
            try
            {
                myDataChannelOut.SendMessage(compressed);
            }
            catch
            {
                print(WebRTCManager.signaler.LocalPeerId + ": Reset Connection because data couldn't be send.");
        ...
        {
            var srlzr= new UnitySerializer();
            srlzr.Serialize(x);
            srlzr.Serialize(y);
            int offset = 0;
            pointerArray[0] = 0x68;
            offset += 1;
            Buffer.BlockCopy(srlzr.ByteArray, 0, pointerArray, offset,  srlzr.ByteArray.Length);
            offset += srlzr.ByteArray.Length;
            var compressed = Compress(pointerArray);
            myDataChannelOut.SendMessage(compressed); // TODO bandwidth optimization
            
        }
        
        public static byte[] Compress(byte[] data)
        {
            MemoryStream output = new MemoryStream();    // CompressionLevel.Optimal: 184 to 55 KB
        ...
        {
            var uncompressed = Decompress(message);
            byte messageType = uncompressed[0];
            if (messageType == CommonParameters.alivePacket)
            {
                //print("received alive packet.");
                alivePacketCount++;
            }
            
            if (DataChannelName == "LocalRemote")
            {
                //AnnotationTasks.Enqueue(message);
                //print(System.Text.Encoding.UTF8.GetString(message));
                //print(message.Length);
                //print("Compressed Len=" + message.Length);
                //print("Type:" + messageType + " Len=" + uncompressed.Length);
                if (messageType == (byte) CommonParameters.codeHandData)
                {
                    UnitySerializer dsrlzr = new UnitySerializer(uncompressed);
                    byte temp = dsrlzr.DeserializeByte();
                    //print("Gesture arrived");
                    //incomingGesture = dsrlzr;
                    byte[] b = new byte[uncompressed.Length - 1];
                    Buffer.BlockCopy(dsrlzr.ByteArray, 1, b, 0,uncompressed.Length - 1);
                    incomingGesture = b;
                }
                else if (messageType ==  (byte)0x67)
                {
                    if (incomingAnnotationPacket == null)
                    {
                        incomingAnnotationPacket =
                            new byte[CommonParameters.incomingAnnotationPacketSize]; // TODO: do this elsewhere, this should not be hardcoded
                    }
                    incomingAnnotationPacket = uncompressed;
                    //print("Annotation arrived"); 
                    
                }
                else if (messageType ==  (byte)0x68)
                {
                    if (incomingPointerPacket == null)
                    {
                        incomingPointerPacket =
                            new byte[CommonParameters.incomingPointerPacketSize]; // TODO: do this elsewhere, this should not be hardcoded
                    }
                    incomingPointerPacket = uncompressed;
                ...
                //byte[] other = dsrlzr.ByteArray;
                //print("Other size =" + other.Length);
                //incomingGesture = other;
                //incomingGesture = message;
            }
            else if(DataChannelName == "LocalComputerLocal") 
            {
                //string text = System.Text.Encoding.UTF8.GetString(message); 
                //textToProcess = text; 
                //print("Message Received: " + message.Length);
                //Buffer.BlockCopy(message, 0,  IncomingMeshRenderer.incomingDepthPacket, 0, message.Length);
                if (incomingDepthPacket == null)
                {
                    incomingDepthPacket =
                        new byte[CommonParameters
                            .depthPacketSize]; // TODO: do this elsewhere, this should not be hardcoded
                }

                //print("before: "+ message.Length / 1000);
                if(messageType == CommonParameters.codeDepthPacket)
                    incomingDepthPacket = uncompressed;
                else if (messageType == CommonParameters.codeTrackerPacket)
                {
                    UnitySerializer serializer = new UnitySerializer(uncompressed);
                    var id = serializer.DeserializeByte();
                    trackerPosition = serializer.DeserializeVector3();
                    trackerRotation = serializer.DeserializeQuaternion();
                    //print("ID " + id);
                    print("Tracker Position " + trackerPosition);
                    print("Tracker Rotation " + trackerRotation);
                } else if (messageType == CommonParameters.codeShutdownPacket)
                {
                    print("Shutdown received.");
                    //doShutdown = true;
                }
                //print("Packet received size: "+ uncompressed.Length / 1000);
            } else if (DataChannelName == "LocalComputerRemote"){
                 if (incomingDepthPacket == null)
                 {
                     incomingDepthPacket = new byte[CommonParameters.depthPacketSize]; // TODO: do this elsewhere, this should not be hardcoded
                 }
                ...
                } else if (messageType == CommonParameters.codeSelectCamera)
                {
                    ...
                    print("Select Camera: " + selectCamera);
                ...
                /*
                if (text == "alignHologram")
                {
                    //print(TrackerCameraCube + ", " );

                    var position = DepthData.trackerPosition;
                    var rotation = DepthData.trackerRotation;
                    UnitySerializer srlzr = new UnitySerializer();
                    srlzr.Serialize(CommonParameters.codeTrackerPacket);
                    srlzr.Serialize(position);
                    srlzr.Serialize(rotation);
                    var trackerTransform = srlzr.ByteArray;
                    var compressed = Compress(trackerTransform);
                    SendData(compressed);
                    print("Sending position" + position);
                    print("Sending rotation" + rotation);
                }
                */
             }
        }
